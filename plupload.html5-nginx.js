...
(function(window, document, plupload, undef) {
	 ...
	 *
	 * @static
	 * @class plupload.runtimes.Html5nginx
	 * @extends plupload.Runtime
	 */
	plupload.runtimes.Html5nginx = plupload.addRuntime("html5nginx", {
		/**
			...
			uploader.bind("UploadFile", function(up, file) {
				...
				function sendBinaryBlob(blob) {
					var chunk = 0, loaded = 0,
						...
						sessionID=new Date().getTime()+'_'+jQuery.base64.encode(file.name).replace(/\+|=|\//g, '');
					...
					function uploadNextChunk() {
						...
						function prepareAndSend(bin) {
							...
							xhr.open("post", up.settings.url, true);
							
							mimeType = plupload.mimeTypes[file.name.replace(/^.+\.([^.]+)/, '$1').toLowerCase()] || 'application/octet-stream';
							
							xhr.setRequestHeader('Content-Disposition', 'attachment; filename="' + sessionID + '.' + file.name.split('.').pop() + '"');
							xhr.setRequestHeader('Content-Type', 'application/octet-stream');
							xhr.setRequestHeader('Content-Range', 'bytes '+loaded+'-'+(loaded+curChunkSize-1)+'/'+(file.size));
